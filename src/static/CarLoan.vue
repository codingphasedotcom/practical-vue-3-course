        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input id="downPayment" v-model.number="downPayment" type="number" min="0" value="5000"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
            </div>

            <div>
              <label for="interestRate" class="block text-sm font-medium text-gray-700">Interest Rate (%)</label>
              <input id="interestRate" v-model.number="interestRate" type="number" min="0" step="0.01" value="4.5"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
            </div>

            <div>
              <label for="loanTerm" class="block text-sm font-medium text-gray-700">Loan Term (months)</label>
              <input id="loanTerm" v-model.number="loanTerm" type="number" min="1" max="120" value="60"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
            </div>
          </div>
...
// initialize and update this charts
const updateCharts = () => {
  if (pieChart) {
    pieChart.destroy();
  }
...
const monthlyPayment = computed(() => {
  ...
  return (
    (loanAmount.value *
      monthlyRate *
      Math.pow(1 + monthlyRate, loanTerm.value)) /
